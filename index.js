// ==========================================
// Baytna Render Backend Server - FULL VERSION
// ==========================================
...
// دالة حساب المستوى (تطابق الأندرويد)
...
app.get("/api/leaderboard", async (req,res)=>{
    ...
    try {
        const snapshot = await db.collection("users")
            ...
            .orderBy(field, "desc").limit(Number(limit)).get();
...
app.get("/api/users/search", async(req,res)=>{
    ...
    try {
        ...
        const results = snapshot.docs.map(doc=>({ userId: doc.id, ...doc.data() }))
            .filter(u => (u.name || "").toLowerCase().includes(query) || u.userId.includes(query));
        res.json(results);
    } catch(error){ res.status(500).send(error.message); }
});
    ...
    try {
        ...
        for(const userDoc of usersSnapshot.docs){
            ...
            for(const type of types){
                ...
                let cycleKey;
                if (type === "DAILY") cycleKey = now.getFullYear() * 1000 + dayOfYear;
                else if (type === "WEEKLY") cycleKey = now.getFullYear() * 100 + getWeekNumber(now);
                else cycleKey = now.getFullYear() * 100 + now.getMonth();

                const docId = `user_${userDoc.id}_${type}_cycle_${cycleKey}`;
                const ref = db.collection("challenge_progress").doc(docId);
...
app.get("/api/challenges/pending-with-users", async(req,res)=>{
    try {
        ...
        const result = snapshot.docs.map(doc => ({
            ...
            user: usersMap[doc.data().userId] || null
        }));
        res.json(result);
...
app.post("/api/challenges/approve", async(req,res)=>{
    ...
    try {
        await db.runTransaction(async(t)=>{
            ...
            t.update(userRef, { points: newTotal, level: calculateLevel(newTotal), weeklyPoints: (uSnap.data().weeklyPoints || 0) + points });
...
app.post("/api/user/daily-login", async(req,res)=>{
    ...
    try {
        const result = await db.runTransaction(async(t)=>{
            ...
            t.update(userRef, { points: newTotal, streakCount: streak, lastLoginDate: now, level: calculateLevel(newTotal) });
            const logRef = db.collection("points_logs").doc();
            t.set(logRef, { logId: logRef.id, userId, points, actionType: "daily_login", description: `تسجيل دخول يومي - ${streak} يوم`, timestamp: now, source: "server" });
...
app.post("/api/rewards/open-box", async(req,res)=>{
    ...
    try {
        ...
        await db.runTransaction(async(t)=>{
            ...
            if(!uSnap.exists) throw new Error("المستخدم غير موجود");
...
// ===============================
// 5. Store & Heavy Admin Operations
// ===============================

app.get("/api/store/stats", async (req, res) => {
    try {
        const [items, sales] = await Promise.all([db.collection("store_items").get(), db.collection("store_purchases").get()]);
        let totalRev = 0; sales.forEach(d => totalRev += (d.data().price || 0));
        res.json({ totalItems: items.size, totalRevenue: totalRev, todaySales: sales.docs.filter(d => d.data().timestamp > new Date().setHours(0,0,0,0)).length });
    } catch (e) { res.status(500).send(e.message); }
});
...
app.post("/api/certificates/issue", async (req, res) => {
    ...
    try {
        const winnerId = db.collection("monthly_winners").doc().id;
        await db.collection("monthly_winners").doc(winnerId).set({
            winnerId, userId, title, timestamp: Date.now(), type: type || "manual",
            monthYear: new Date().toLocaleString('ar-EG', { month: 'long', year: 'numeric' })
        });
        await logAdminAction("System", "issue_certificate", `Issued certificate to ${userId}`);
        res.json({ success: true });
    } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.post("/api/admin/force-logout-all", async (req, res) => {
    ...
    try {
        ...
        await batch.commit();
        await logAdminAction(adminName, "force_logout_all", "Forced logout for all users");
        res.json({ success: true });
    } catch (e) { res.status(500).json({ error: e.message }); }
});
